...
template <typename T> inline void CenterOrigin(T& object) { object.setOrigin(object.getGlobalBounds().getCenter()); }
...
inline std::string GetEngineVersion() { return std::string(std::to_string(REN_VERSION_MAJOR) + "." + std::to_string(REN_VERSION_MINOR) + "." + std::to_string(REN_VERSION_PATCH)); }
} // namespace Ren
