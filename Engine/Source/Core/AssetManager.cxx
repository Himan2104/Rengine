...
#include "Rengine/core/AssetManager.hxx"
#include <iostream>
...
void AssetManager::loadTexture(std::string_view name, std::string filepath)
{
    sf::Texture temp;
    if (!temp.loadFromFile(filepath))
        std::cout << name << " failed to load" << std::endl;
    else
        _textures.insert(std::make_pair(name, temp));
}
...
void AssetManager::loadSFX(std::string_view name, std::string filepath)
{
    sf::SoundBuffer temp;
    if (!temp.loadFromFile(filepath))
        std::cout << name << " failed to load" << std::endl;
    else
        _sfx.insert(std::make_pair(name, temp));
}

void AssetManager::loadMusic(std::string_view name, std::string filepath)
{
    _music.insert(std::make_pair(name, filepath));
}

sf::Texture& AssetManager::getTexture(std::string_view name)
{
    if (_textures.count(name.data()))
        return _textures.at(name.data());
    else
        return _textures.at("texture_not_found");
}

sf::Font& AssetManager::getFont(std::string_view name)
{
    if (_fonts.count(name.data()))
        return _fonts.at(name.data());
    else
        return _fonts.at("default_font");
}

sf::SoundBuffer& AssetManager::getSFX(std::string_view name)
{
    if (_sfx.count(name.data()))
        return _sfx.at(name.data());
    else
        return _sfx.at("sfx_not_found");
}

std::string& AssetManager::getMusic(std::string_view name)
{
    if (_music.count(name.data()))
        return _music.at(name.data());
    else
        return _music.at("music_not_found");
}
} // namespace Ren
