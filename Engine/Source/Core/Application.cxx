#include "SFML/Window/Keyboard.hxx"
#include "SFML/Window/WindowEnums.hxx"
#include <Rengine/Core/Application.hxx>
#include <Rengine/Core/AssetManager.hxx>
#include <Rengine/Core/LevelManager.hxx>
#include <typeinfo>
#include <utility>
...
Application::Application(ApplicationProperties app_properties)
    ...
    , _debugText(_font)
{
    _window.create(_properties.settings.video_mode, _properties.name + " v" + _properties.GetVersionString(),
                   _properties.settings.fullscreen ? sf::State::Fullscreen : sf::State::Windowed);
    ...
    if (_font.openFromFile("C:/Windows/Fonts/consola.ttf"))
    {
...
void Application::Run(Level* firstState)
{
    ...
    while (_isRunning)
    {
        ...
        {
            if (event->is<sf::Event::Closed>()) _isRunning = false;
            if (const auto* keyPressed = event->getIf<sf::Event::KeyPressed>())
                if (keyPressed->scancode == sf::Keyboard::Scancode::Escape) Debug::log("OpenConsole");
        }
